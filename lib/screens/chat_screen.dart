...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Stream<DocumentSnapshot<Map<String, dynamic>>>? _chatMetadataStream;
  StreamSubscription<bool>? _bookingAccessSubscription;
  Timer? _typingResetTimer;
  bool _isSending = false;
  bool _isRecording = false;
  bool _isUploading = false;
  bool _hasFullChatAccess = false;
  ...
  void initState() {
    ...
    _chatMetadataStream = _chatService.getChatDocumentStream(_chatId);
    _chatService.ensureChatDocument(otherUserId: otherUserId);
    _chatService.updatePresence(
      chatId: _chatId,
      otherUserId: otherUserId,
      isOnline: true,
    );
    _bookingAccessSubscription = _bookingService
        .watchBookingAccess(otherUserId)
        .listen(
          (hasAccess) {
            if (!mounted) return;
            setState(() {
              _hasFullChatAccess = hasAccess;
  ...
  void dispose() {
    ...
    _bookingAccessSubscription?.cancel();
    if (_chatReady && _chatService.currentUserId.isNotEmpty) {
      _chatService.updateTyping(
        chatId: _chatId,
  ...
  Future<void> _markMessagesAsSeenAndResetCount() async {
    try {
      ...
      debugPrint('[ChatScreen] Error marking seen: $e');
    }
  }
  ...
  Future<void> _sendMessage() async {
    ...
    if (!_hasFullChatAccess) {
      final moderation = ChatModerationService.validateLimitedMessage(
        messageText,
      );
      if (!moderation.allowed) {
        _showPolicyWarning(moderation.warning ?? 'Message blocked.');
        return;
      }
    }
  ...
  void _showPolicyWarning(String message) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
  ...
  Widget _buildPresenceLine() {
    final fallback = widget.otherUserRole ?? 'user';
    final stream = _chatMetadataStream;
    if (stream == null) {
      return Text(
        fallback,
        style: GoogleFonts.inter(
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final isOnline = data['online_${widget.otherUserId}'] == true;
        final text = isTyping
            ? 'typing...'
            : _checkingAccess
            ? fallback
            : _hasFullChatAccess
            ? (isOnline ? 'Full chat unlocked · online' : 'Full chat unlocked')
            : 'Limited chat';
        return Text(
          text,
          style: GoogleFonts.inter(
            fontSize: 11,
            color: isTyping || _hasFullChatAccess
  ...
  Widget _buildInputSection() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          GestureDetector(
            onTap: _hasFullChatAccess
                ? _showMediaOptions
                : () => _showPolicyWarning(
                    'Image and file sharing unlock after booking.',
                  ),
            child: Icon(
              _hasFullChatAccess
          ...
          GestureDetector(
            onTap: _messageController.text.trim().isEmpty
                ? (_hasFullChatAccess
                      ...
                      : () => _showPolicyWarning(
                          'Voice notes unlock after booking.',
                        ))
  ...
  void _showMediaOptions() {
    if (!_hasFullChatAccess) {
      _showPolicyWarning('Media sharing unlocks after booking.');
      return;
    }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _mediaOption(Icons.camera_alt_rounded, 'Camera', Colors.blue, () {
              Navigator.pop(context);
              _takePhoto();
            }),
            const SizedBox(height: 8),
