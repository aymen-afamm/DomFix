...
class _FindProsScreenContentState extends State<FindProsScreenContent>
  ...
  final List<ProProfile> _profiles = [
    ProProfile(
      ...
      description:
          'Specializing in smart home integration and complex rewiring. Available for emergency calls 24/7.',
      imageUrl:
          'https://via.placeholder.com/400x600/2A3040/FFFFFF?text=Electrician',
    ),
    ProProfile(
      name: 'Sarah K.',
      profession: 'Plumber',
      ...
      description:
          'Expert in leak detection and pipe repair. Fast response time for emergencies.',
  ...
  void _animateSwipe(bool isRight) {
    ...
    _swipeAnimation = Tween<Offset>(
      begin: _dragOffset,
      end: Offset(isRight ? screenWidth * 1.5 : -screenWidth * 1.5,
          _dragOffset.dy),
    ).animate(CurvedAnimation(
      parent: _swipeController,
      curve: Curves.easeOut,
  ...
  Widget _buildCardStack() {
    return Stack(
      children: [
        if (_currentCardIndex + 2 < _profiles.length) _buildBackgroundCard(2),
        if (_currentCardIndex + 1 < _profiles.length) _buildBackgroundCard(1),
        _buildActiveCard(),
      ],
    );
  }
